...
def prepare_data():
    # do not modify
    df_train = pd.read_csv(r"D:\AI_hw_data\hw2\data\IMDB_train.csv")
    df_test = pd.read_csv(r"D:\AI_hw_data\hw2\data\IMDB_test.csv")
    '''./data/IMDB_test.csv'''
...
def second_part(model_type, df_train, df_test, N):
    ...
    if model_type == 'ngram':
        ...
        # pad填充 and truncate 結斷 for this batch
        def collate_fn(data):
...
def train(model_type, model, train_dataloader, test_dataloader, optimizer, loss_fn, config):
    ...
    for epoch in tqdm(range(config['epochs'])):
        ...
        with tqdm(total=len(train_dataloader), desc="Training") as pbar:
            for input_data, label in train_dataloader:
                input_data, label = input_data.to(config['device']), label.to(config['device'])
        ...
        with torch.no_grad():
            for input_data, label in test_dataloader:
                input_data, label = input_data.to(config['device']), label.to(config['device'])
